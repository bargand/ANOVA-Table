function calculateSummaryStats() {
  ...
  for (let t = 1; t <= maxTreatments; t++) {
      ...
      for (let i = 1; i <= locationCount; i++) {
          ...
          let meanValue = t <= treatCount ? parseFloat(document.getElementById(`loc${i}treat${t}mean`).innerText) || 0 : 0;
          rowTotal += meanValue;
  ...
  for (let i = 1; i <= locationCount; i++) {
      let treatCount = parseInt(document.getElementById(`treatmentCount${i}`).value);
      let repCount = parseInt(document.getElementById(`replicationCount${i}`).value);
      ...
      let TrSS_Location = (treatmentTotals_Location.reduce((sum, val) => sum + val ** 2, 0) / repCount) - CF_Location;
      let RSS_Location = (replicationTotals_Location.reduce((sum, val) => sum + val ** 2, 0) / treatCount) - CF_Location;
  ...
  <div class="formula-box">
      <p><strong>Correction Factor (CF):</strong> (${grandTotal}² / ${N}) = <b>${correctionFactor.toFixed(2)}</b></p>
      <p><strong>Total Sum of Squares (TSS):</strong> (Sum of Squares of all values - CF) = <b>${totalSumOfSquares.toFixed(2)}</b></p>
      <p><strong>Treatment Sum of Squares (TrSS):</strong> ((Sum of Squares of Treatment Totals / Locations) - CF) = <b>${treatmentSumOfSquares.toFixed(2)}</b></p>
      <p><strong>Replication Sum of Squares (RSS):</strong> ((Sum of Squares of Replication Totals / Treatments) - CF) = <b>${replicationSumOfSquares.toFixed(2)}</b></p>
      <p><strong>Error Sum of Squares (ESS):</strong> (TSS - (TrSS + RSS)) = <b>${errorSumOfSquares.toFixed(2)}</b></p>
  </div>`;
